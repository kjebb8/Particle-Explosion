...
bool Screen::init() {
  ...
  m_renderer = SDL_CreateRenderer(m_window, -1, SDL_RENDERER_PRESENTVSYNC); //Drawing on the screen raster scan
  ...
  m_buffer2 = new Uint32[SCREEN_WIDTH * SCREEN_HEIGHT]; //1 byte for each R/G/B/A per pixel (32bytes/pixel), so this buffer holds info for entire screen
  //Ideally would handle an exception here if allocation fails
